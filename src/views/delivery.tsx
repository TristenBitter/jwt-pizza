...
import { JWTPayload, Order, OrderItem } from "../service/pizzaService";

export default function Delivery() {
  const navigate = useNavigate();
  const location = useLocation();

  // Fallback order has items (not pizzas)
  ...
  async function verify() {
    try {
      ...
      setJwtPayload(r); // r satisfies JWTPayload
    } catch (e) {
      const errMsg =
      ...
      // JWTPayload.payload is a string, so keep it a string:
      setJwtPayload({
        message: errMsg,
  ...
  const totalPrice = order.items?.reduce(
    (acc: number, curr: OrderItem) => acc + curr.price,
    0
  );
    ...
    <View title="Here is your JWT Pizza!">
      <div className="my-4 flex max-w-xl justify-center items-center flex-col py-8 px-4 sm:px-6 lg:px-8">
        ...
        <div className="my-4 text-lg text-orange-200 text-start grid grid-cols-5 gap-2">
          ...
          <div className="col-span-4">{order.items?.length ?? 0}</div>
