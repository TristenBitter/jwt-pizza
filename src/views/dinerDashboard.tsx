...
export default function DinerDashboard(props: Props) {
  ...
  async function updateUser() {
    ...
    // Persist to backend
    const persisted = await pizzaService.updateUser(updatedUser);
    // Update app state
    props.setUser(persisted);

    // Close modal (with slight delay for Preline state sync)
    setTimeout(() => {
                    ...
                    <div className="overflow-hidden">
                      <table className="min-w-full divide-y divide-gray-200">
                        <thead className="uppercase text-neutral-100 bg-slate-400 border-b-2 border-gray-500">
                          <tr>
                            <th className="px-6 py-3 text-start text-xs sm:text-sm font-medium">
                              ID
                            </th>
                            <th className="px-6 py-3 text-start text-xs sm:text-sm font-medium">
                              Price
